...
class FanfanStepInPlaceNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("lift_height", 0.040)
        self.declare_parameter("front_lift_height_gain", 1.15)
        self.declare_parameter("rear_lift_height_gain", 1.08)
        self.declare_parameter("front_calf_lift_extra", 0.090)
        self.declare_parameter("rear_calf_lift_extra", 0.100)
        self.declare_parameter("rear_thigh_lift_extra", 0.035)
        self.declare_parameter("front_thigh_lift_scale", 0.58)
        self.declare_parameter("active_pre_unload_m", 0.012)
        self.declare_parameter("stomp_touchdown_push_m", 0.002)
        self.declare_parameter("front_stomp_touchdown_push_m", 0.002)
        self.declare_parameter("front_companion_press_m", 0.005)
        self.declare_parameter("front_companion_calf_push", 0.018)
        self.declare_parameter("support_stand_tall_m", 0.008)
        self.declare_parameter("diagonal_support_extra_m", 0.010)
        ...
        self.declare_parameter("front_calf_min_rad", -1.45)
        ...
        self.declare_parameter("max_target_rate_rad_s", 4.0)
        ...
        self.active_pre_unload_m = float(self.get_parameter("active_pre_unload_m").value)
    ...
    def desired_body_shift(self, swing_leg: str, event_s: float) -> tuple[float, float, float]:
        if event_s < 0.18:
            gate = self.smoothstep(event_s / 0.18)
        elif event_s > 0.88:
            gate = self.smoothstep((1.0 - event_s) / 0.12)
        else:
            gate = 1.0
    ...
    def active_pre_unload_gate(self, event_s: float) -> float:
        if event_s < 0.18:
            return self.smoothstep(event_s / 0.18)
        if event_s < 0.34:
            return 1.0
        if event_s < 0.52:
            return self.smoothstep((0.52 - event_s) / 0.18)
        return 0.0
    ...
    def swing_lift_gate(self, event_s: float) -> tuple[float, str]:
        if event_s < 0.16:
            return 0.0, "SHIFT"
        if event_s < 0.42:
            return self.smoothstep((event_s - 0.16) / 0.26), "LIFT"
        if event_s < 0.62:
            return 1.0, "HOLD"
        if event_s < 0.82:
            return self.smoothstep((0.82 - event_s) / 0.20), "DOWN"
        if event_s < 0.92:
            return 0.0, "TAP"
        return 0.0, "SETTLE"

    def stomp_touchdown_gate(self, event_s: float) -> float:
        if event_s < 0.82 or event_s > 0.94:
            return 0.0
        rise = self.smoothstep((event_s - 0.82) / 0.04)
        fall = self.smoothstep((0.94 - event_s) / 0.06)
    ...
    def build_step_target(self, phase: float, warm: float):
        ...
        pre_unload_gate = self.active_pre_unload_gate(event_s)
        for leg in LEG_ORDER:
            ...
            if is_active:
                ...
                z += warm * self.active_pre_unload_m * pre_unload_gate
